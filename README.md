...
### 2) 시험팀 COM 을 다른 사람이 같이 보기 (연결.bat)

GitHub Pages 에는 중계 서버가 없습니다. 공유는 시험팀 PC 의 `연결.bat` 이 맡습니다.

1. 시험팀: `연결.bat` → `http://127.0.0.1:8765/` 에서 COM 연결 → [공유 시작]
2. 개발팀: 시험팀이 준 `join.html?room=...` 링크 (시험팀 IP:8765)

사내망에 직접 못 붙는 출장/사외:

1. **양쪽 모두 사내 VPN** 을 켭니다.
2. 시험팀은 위와 같이 `127.0.0.1` 에서 공유합니다. (COM 은 VPN 과 무관)
3. 개발팀 링크는 **VPN 이 준 주소** 여야 합니다. GitHub Pages 의 join.html 이 아닙니다.

VPN 없이 GitHub Pages 만 열면, 출장자는 자기 USB 콘솔만 되고 사무실 사람은 그 로그를 볼 수 없습니다.
...
- GitHub Pages 만으로는 시험팀 PC 로 원격 공유가 되지 않습니다.
