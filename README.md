...
| 장소 | 시험팀 | 개발팀 |
|------|--------|--------|
| **사내** | `연결.bat` 실행 → `http://127.0.0.1:8765/` | 시험팀이 준 **사내 IP:8765** 링크 |
| **사외 / 출장** | 위 공식 주소에서 [공유 시작] | 시험팀이 준 **GitHub 공유 링크** |

![사내는 연결.bat, 사외는 GitHub Pages](docs/readme-where.png)

사내에서도 GitHub 링크만 쓸 수는 있습니다. 다만 회사망이 P2P 를 막으면 접속이 실패하거나, 콘솔 내용이 사외 중계를 탈 수 있습니다. 사내 로그를 망 안에 두려면 `연결.bat` 을 쓰세요.
...
- 개발팀: `연결.bat` 을 실행하지 않습니다. COM 도 고르지 않습니다. 받은 링크만 엽니다.
...
7. 끝나면 [공유 종료]. 탭이나 `연결.bat` 창을 닫아도 공유가 끊깁니다.
...
3. 나온 공유 링크와 비밀번호를 보냅니다. 개발팀은 `연결.bat` 이 필요 없습니다.
4. **이 탭을 닫으면 공유가 끊깁니다.**
...
- 시험팀 탭과 (`연결.bat` 을 썼다면) 검은 창이 아직 열려 있는지 확인합니다.
- 사내: 링크가 `http://사내IP:8765/...` 인지 봅니다. Pages `github.io` 링크는 사내 전용이 아닙니다.
- 사외: 회사망이 WebRTC 를 막으면 Pages 공유가 실패할 수 있습니다. 그때는 `연결.bat` + VPN 을 씁니다.
...
- 사내 IIS 에 `index.html`, `join.html`, `src/`, `vendor/` 만 올리면 COM 화면은 됩니다. 공유는 여전히 `연결.bat` 입니다.
- `연결.bat` 의 C# 을 바꾼 뒤에는 창을 닫고 다시 실행하세요.
